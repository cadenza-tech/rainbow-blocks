...
def bar
  ...
  suite('Regression: x=%= should not start unterminated percent literal', () => {
    test('should treat x=%= as compound assignment (not percent literal) so following blocks parse', () => {
      // Bug: `%=` mid-expression after `=` started an unterminated percent literal
      // that swallowed the rest of the source. Treat `%=` as compound assignment regardless of context.
      const source = 'x=%=\nif true\nend';
      const pairs = parser.parse(source);
      assertSingleBlock(pairs, 'if', 'end');
    });
