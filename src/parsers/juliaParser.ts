...
export class JuliaBlockParser extends BaseBlockParser {
  ...
  // Like hasUnmatchedBlockOpenerBetween, but treats `end` followed by a binary operator as lastindex
  private hasUnmatchedBlockOpenerBetweenInIndexing(source: string, start: number, end: number, excludedRegions: ExcludedRegion[]): boolean {
    return hasUnmatchedBlockOpenerBetweenInIndexing(source, start, end, excludedRegions, this.keywords.blockOpen, this.juliaHelperCallbacks);
  }

  // Checks whether every unmatched opener in [start, end) is a `begin` filtered as firstindex
  private allUnmatchedOpenersAreFilteredBegins(source: string, start: number, end: number, excludedRegions: ExcludedRegion[]): boolean {
    return allUnmatchedOpenersAreFilteredBegins(source, start, end, excludedRegions, this.keywords.blockOpen, this.juliaHelperCallbacks);
  }

  // Like allUnmatchedOpenersAreFilteredBegins, but also requires an enclosing indexing bracket
  private allUnmatchedOpenersAreFilteredBeginsInsideIndexing(source: string, start: number, end: number, excludedRegions: ExcludedRegion[]): boolean {
    return allUnmatchedOpenersAreFilteredBeginsInsideIndexing(
      source,
      start,
      end,
      excludedRegions,
      this.keywords.blockOpen,
      this.juliaHelperCallbacks
    );
  }

  // Checks whether every unmatched `begin` in [start, end) is the firstindex keyword (followed by `:`)
  private allUnmatchedBeginsAreFirstindex(source: string, start: number, end: number, excludedRegions: ExcludedRegion[]): boolean {
    return allUnmatchedBeginsAreFirstindex(source, start, end, excludedRegions, this.keywords.blockOpen, this.juliaHelperCallbacks);
  }
